        ...
        <tbody id="dataTableJS">
          @foreach ($order as $value)
            <tr class="rowTable{{$value->id}}">
              ...
              <td>
                <button type="button" title="Chi tiết đơn hàng" data-toggle="modal" data-target="#orderDetail" class="btn btn-sm btn-outline-primary showOrderDetailJS"  data-id="{{ $value->id }}">
                  <i class="fas fa-eye"></i>
                </button>
                <button type="button" title="Xóa" data-toggle="modal" data-target="#delOrderDetailModal" class="btn btn-sm ml-2 btn-outline-danger delOrderDetailJS" data-id="{{ $value->id }}" data-name="{{$value->name}}">
...
@push('adminAjax')
  <!-- ajax modal -->
  <script src="/assets/admin/js/order-ajax.js"></script>
@endpush